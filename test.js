...
`;
    ...
    button.addEventListener('click', function () {
        ...
        const antiAfkValue = antiAfkInput.checked;
        const aichatValue = aichatInput.checked;
...
function updatePonyTownLogo() {
  // Pilih gambar yang mau diubah
  const img = document.querySelector('img.pixelart.home-logo');
  if (!img) return console.warn('Logo tidak ditemukan.');

  // Ubah atribut gambar
  img.src = 'https://raw.githubusercontent.com/RandSfk/pt/refs/heads/main/ptbot.png';
  ...
  // Atur parent-nya agar center pakai flexbox
...
function showUpdateNotice(title, titleColor, message, messageColor, link) {
  const avatarURL = 'https://raw.githubusercontent.com/RandSfk/pt/main/randsfk.png';
...
if (currentVersion === requiredVersion) {
  ...
  showUpdateNotice(
    'Update Dibutuhkan',
    '#222222',
    `Versi kamu: ${currentVersion || 'tidak diketahui'}.\nVersi dibutuhkan: ${requiredVersion}`,
    '#444444',
    { text: 'Download Update', url: 'https://example.com/apk' }
  );
}
